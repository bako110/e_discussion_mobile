...
const SECTION_KEYS = Array.from({ length: 10 }, (_, i) => `s${i + 1}`);
  ...
  return (
    <Screen edges={[]}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <Text style={[styles.updated, { color: c.textFaint }]}>{t('privacyDoc.updated')}</Text>
        <Text style={[styles.intro, { color: c.textMuted }]}>{t('privacyDoc.intro')}</Text>

        {SECTION_KEYS.map((key) => (
          <React.Fragment key={key}>
            <Text style={[styles.h2, { color: c.text }]}>{t(`privacyDoc.${key}_title`)}</Text>
            <Text style={[styles.body, { color: c.textMuted }]}>{t(`privacyDoc.${key}_body`)}</Text>
          </React.Fragment>
        ))}
      </ScrollView>
    </Screen>
